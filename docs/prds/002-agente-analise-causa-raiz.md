...
- Relatorios em EM_ANALISE ha mais de `ANALYSIS_STALE_TIMEOUT_MINUTES` minutos (padrao: 30) *(premissa — confirme ou corrija)* sao marcados como INCOMPLETO durante a consulta de deduplicacao — recuperacao de analises estagnadas, complementar a recuperacao do startup
...
- Banco de dados indisponivel para consulta de deduplicacao → registrar erro no log, nao analisar o batch e propagar a falha ao coletor (PRD 001): a marca d'agua nao avanca e os eventos sao reentregues no proximo ciclo
...
- O agente itera ate identificar a causa raiz ou atingir o limite de iteracoes, configuravel via variavel de ambiente `AGENT_MAX_ITERATIONS` (padrao: 25), implementado com `ModelCallLimitMiddleware(run_limit, exit_behavior="end")` no `create_agent` — termino gracioso que permite gerar o relatorio parcial
...
**Edge cases:**
- MCP Server Kubernetes indisponivel → abortar analise, atualizar relatorio para INCOMPLETO e registrar erro no log; como eventos vinculados a INCOMPLETO sao reanalisados (US01), o incidente volta a ser investigado quando os eventos forem reentregues
...
- Erro na API do Claude (rate limit, timeout) → retry automatico do SDK com backoff exponencial (`max_retries=3` no `ChatAnthropic`); esgotadas as tentativas, abortar, atualizar relatorio para INCOMPLETO e registrar erro
...
**Edge cases:**
- PostgreSQL indisponivel no momento da persistencia → retry com backoff; apos falha definitiva, registrar erro no log incluindo o conteudo Markdown gerado (sem fallback em arquivo) e propagar a falha ao coletor (marca d'agua nao avanca); o relatorio EM_ANALISE estagnado e recuperado pelo timeout da deduplicacao
...
- Evento do batch nao referenciado no campo **Eventos** de nenhum problema da resposta → registrar warning no log; o evento fica sem relatorio proprio e sera reanalisado se reentregue
...
| Eventos vinculados a relatorio INCOMPLETO sao reanalisados | Teste automatizado simulando analise inconclusiva seguida de reentrega dos eventos |
...
| Reanalise recorrente de incidentes com relatorio INCOMPLETO consome tokens a cada reentrega dos eventos | Medio | Aceito — analise nao concluida deve ser reavaliada ate o incidente ser resolvido. Mitigacao futura: limite de reanalises por incidente | Aceito |
...
- **2026-06-10:** Falha na consulta de deduplicacao propaga erro ao coletor (PRD 001), que nao avanca a marca d'agua. Motivo: eventos brutos nao sao persistidos; sem segurar a marca d'agua, eventos one-shot seriam perdidos em falhas transitorias.
